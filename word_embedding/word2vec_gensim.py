...
def load_data_from_dir(data_path):
    ...
    for sub_dir in (os.listdir(data_path)):
        path_to_subdir = data_path + '/' + sub_dir
        file_names = os.listdir(path_to_subdir)
        # print(file_names)
        for f_name in file_names:
            file_path = os.path.join(path_to_subdir, f_name)
            # print(os.path.isdir(file_path))
            if f_name.startswith('.') or os.path.isdir(file_path):
                continue
            batch_sentences = load_data_from_file(file_path)
...
def train(data_path= dir_path + "/../data/word_embedding/training", load_data=load_data_from_dir,
          model_path= dir_path + "/../models/word2vec.LM.model"):
    """
...
def test(model_path= dir_path + "/../models/word2vec.model", word="thu_nhập"):
    """
    Test word2vec model
...
if __name__ == '__main__':
    # train()
    test(model_path= dir_path + "/../models/word2vec.LM.model")